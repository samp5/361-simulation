...
static int test_state_empty(){
  ...
  // table ids
  ASSERT(s->table_ids->len(s->table_ids) == 0, "tables should be empty");
...
static int test_waitstaff_single(){
  ...
  ASSERT(w1.tables->len(w1.tables) == 1, "waiter should have 1 table");
  for (int i = 0; i < BORSHT_TYPES; i++){
...
static int test_waitstaff_many(){
  ...
  for (int i = 0; i < 20; i++){
    ...
    ASSERT(w_i.tables->len(w_i.tables) == 5, "waiter should have 5 tables");

    for (int j = 0; j < 5; j++){

      table t_j;

      FASSERT(w_i.tables->get_at(w_i.tables, j, (void*)&t_j) == 0, "waiter %d should have table at index %d", i, j);
...
static int test_waitstaff_many_unbalanced(){
  ...
  ASSERT(w_i.tables->len(w_i.tables) == 6, "waiter should have 6 tables");

  for (int j = 0; j < 6; j++){

    table t_j;

    FASSERT(w_i.tables->get_at(w_i.tables, j, (void*)&t_j) == 0, "waiter %d should have table at index %d", 0, j);
    FASSERT(t_j.id == start, "table should have id %d, got %d", start, t_j.id);
    FASSERT(t_j.current_status == Clean, "table id %d should be Clean", start);
