...
int pop(vector *vec, void *element) {
  ...
  memcpy(element, vec->arr + ((vec->size - 1) * vec->elem_size),
         vec->elem_size);
  vec->size -= 1;
  return 0;
}
 ...
 * -1 is returned on error
 *
 * `element` holds element at index `index` on success
 */
int get_at(vector *vec, size_t index, void *element) {
 ...
 * -1 is returned on error
 *
 * `element_ptr` holds a pointer to element at index `index` on success
 */
int get_mut_at(vector *vec, size_t index, void **element_ptr) {
  if (vec == NULL || index >= vec->size) {
    return -1;
  }
  *element_ptr = vec->arr + (index * vec->elem_size);
  return 0;
}
